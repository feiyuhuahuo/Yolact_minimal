...
if cfg.image is not None:
    ...
    print('\nFinished, saved in: results/onnx_images.')
...
# detect videos
    ...
    video_writer = cv2.VideoWriter(f'results/onnx_videos/{name}', cv2.VideoWriter_fourcc(*"mp4v"), target_fps,
    ...
    if not cfg.real_time:
        print(f'\n\nFinished, saved in: results/onnx_videos/{name}')
